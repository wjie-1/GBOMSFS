function [Prior,PriorN,Cond,CondN]=MLKNN_train1(train_data,train_target,Num,Smooth)
...
%Computing Prior and PriorN
    for i=1:num_class
        ...
        % PriorN(i,1)=1-Prior(i,1);                                  %%%% PriorN(i,1)!=1-Prior(i,1); 此处如果不考虑缺失位置，则num_training应该改成已知的标签的个数
        temp_NCi=sum(train_target(i,:)==(-1*ones(1,num_training)));  %%%%
        PriorN(i,1)=(Smooth+temp_NCi)/(Smooth*2+num_training);       %%%%
    end
...
%Computing Cond and CondN
    ...
    for i=1:num_class
        ...
        for j=1:Num+1
            Cond(i,j)=(Smooth+temp_Ci(i,j))/(Smooth*(Num+1)+temp1); % k nearest neighbors of (an instance in Ci) will belong to Ci , is stored in Cond(i,k+1)
            CondN(i,j)=(Smooth+temp_NCi(i,j))/(Smooth*(Num+1)+temp2); % k nearest neighbors of (an instance not in Ci) will belong to Ci, is stored in CondN(i,k+1)
        end
    end              